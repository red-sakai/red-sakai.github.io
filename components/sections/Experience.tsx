...
export function ExperienceSection() {
    ...
    return (
      <div className="relative overflow-hidden rounded-md border border-slate-200/80 bg-slate-50 transition-colors duration-300 group-hover:border-slate-300 dark:border-white/10 dark:bg-white/[0.04] dark:group-hover:border-white/15">
        <div
          ...
          className={`group/carousel relative aspect-video w-full focus-visible:outline-none ${current ? "cursor-zoom-in" : ""}`}
        >
          ...
          {current && (
            <span className="pointer-events-none absolute inset-0 flex items-center justify-center bg-black/0 text-sm font-semibold text-white opacity-0 transition group-hover/carousel:bg-black/40 group-hover/carousel:opacity-100">
              Click to enlarge
            </span>
          )}
        </div>
      ...
      <article key={group.organization} className={cardClass}>
        ...
        <ul className="-my-1 flex flex-col divide-y divide-slate-100 dark:divide-white/5">
          {group.positions.map((pos, posIdx) => (
            <li key={`${group.organization}-${pos.title}-${posIdx}`} className="flex flex-col gap-1 py-3 first:pt-1 last:pb-1">
  ...
  const renderDefaultCards = () =>
    filtered.map((item, index) => (
      <article key={`${item.category}-${item.title}-${index}`} className={cardClass}>
